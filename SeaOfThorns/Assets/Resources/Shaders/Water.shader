    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (v2f i) : SV_Target
            {
				...
				//float4 depthnormal = tex2Dproj(_CameraDepthNormalsTexture, (i.screenPosition/i.screenPosition.w));
                //float existingDepth01= depthnormal.z + depthnormal.w * .2 ;//得到01线性的深度
//
...
//
				...
				float4 waterColor = lerp(_DepthGradientShallow, _DepthGradientDeep, .5);
				//
				//// Retrieve the view-space normal of the surface behind the
				//// pixel we are currently rendering.
				//float3 existingNormal = tex2Dproj(_CameraDepthNormalsTexture, UNITY_PROJ_COORD(i.screenPosition));
				i.distortUV.xy *= 10.;
				...
				// amount consistent.
				float3 normalDot = saturate(dot(float3(0,0,0), i.viewNormal));
				float foamDistance = lerp(_FoamMaxDistance, _FoamMinDistance, normalDot);
				float foamDepthDifference01 = saturate(1 / foamDistance);
				...
				return alphaBlend(surfaceNoiseColor, waterColor) * shadow;
            }
            ENDCG
        }
    }
